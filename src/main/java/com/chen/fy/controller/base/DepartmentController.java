...
public class DepartmentController extends BaseController {
	public void index() {
		...
		if(StringUtils.isEmpty(key)) {
			personPage = Department.dao.paginate(getParaToInt("p", 1), 10, "select d.*,p.name dutyName ", "from fy_base_department d left join fy_base_person p  on d.duty_person = p.id  order by d.id desc");
		}else {
			personPage = Department.dao.paginate(getParaToInt("p", 1), 10, "select d.*,p.name dutyName ", "from fy_base_department d left join fy_base_person p  on d.duty_person = p.id  where part_name like ? order by d.id desc","%"+key+"%");
	...
	public void searchDepartJson() {
		...
		setAttr("keyWord", key);
		if(StringUtils.isEmpty(key)) {
			personPage = Department.dao.paginate(getParaToInt("p", 1), 6, "select d.*,p.name dutyName ", "from fy_base_department d left join fy_base_person p  on d.duty_person = p.id  order by d.id desc");
		}else {
			personPage = Department.dao.paginate(getParaToInt("p", 1), 6, "select d.*,p.name dutyName ", "from fy_base_department d left join fy_base_person p  on d.duty_person = p.id  where part_name like ? order by d.id desc","%"+key+"%");
			setAttr("append", "keyWord="+key);
		}
