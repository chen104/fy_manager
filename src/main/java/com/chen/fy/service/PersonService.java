...
public class PersonService {
	...
	public Page<Person> paginate(int pageNum) {
		...
		return dao.paginate(pageNum, 10, "select p.* ,d.part_name department", "from fy_base_person p left join fy_base_department d on p.department_id = d.id order by p.id desc");
	}
	...
	public Page<Person> paginate(int pageNum ,String keyWord) {
		...
		return dao.paginate(pageNum, 10, "select p.* ,d.part_name department", "from fy_base_person p left join fy_base_department d on p.department_id = d.id where p.name  like ?  order by p.id desc","%"+keyWord+"%");
	}
	...
	public Page<Person> paginate(int pageNum ,String keyWord,int pageSize) {
		...
		return dao.paginate(pageNum, pageSize, "select *", "from fy_base_person  where name  like ?  order by id desc","%"+keyWord+"%");
	}
