...
import com.chen.fy.model.LoginFailRecord;
...
public class FyLoginService {
    ...
    public Ret login(String userName, String password, boolean keepLogin, String loginIp, BaseController controller) {
        ...
        LoginFailRecord failRecord = CacheKit.get(Constant.loginFailAcountCache, loginAccount.getId());
        if (failRecord != null) {
            if (failRecord.loginNum > 5) {// 锁定
                loginAccount.setStatus(Account.STATUS_LOCK_ID);
            }
        }
        ...
        if (loginAccount.getPassword().equals(hashedPass) == false) {
            Integer loginId = loginAccount.getId();
            LoginFailRecord loginFail = CacheKit.get(Constant.loginFailAcountCache, loginId);
            if (loginFail != null) {
                loginFail = new LoginFailRecord();
                loginFail.FirstLoginTime = new Date();
                CacheKit.put(Constant.loginFailAcountCache, loginId, loginFail);
            }
            loginFail.loginNum = loginFail.loginNum + 1;

            return Ret.fail("msg", "用户名或密码不正确");
        }
        CacheKit.remove(Constant.loginFailAcountCache, loginAccount.getId());
