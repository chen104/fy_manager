package com.chen.fy.permission;
...
public class PermissionAdminService {
	 ...
	 * 替换控制器前缀，界面显示时更加美观
	 *
	 * 例子： replaceControllerPrefix(permissionPage, "com.jfinal.club._admin.",
	...
	/**
	 * 同步 permission 获取后台管理所有 actionKey 以及 controller，将数据自动写入 permission 表
	...
	public Ret sync() {
		...
		for (String actionKey : allActionKeys) {
			...
			if (actionKey.startsWith("/admin")) {
				...
				String sql = "select * from permission where actionKey=? and controller = ?";
