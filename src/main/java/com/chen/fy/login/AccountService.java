...
public class AccountService {
	...
	public Account getByNickName(String nickName, String columns) {
		...
		return dao.findFirst("select " + columns + " from account where nickName=?  and status=?  limit 1", nickName,
				Account.STATUS_OK);
	}
	...
	public Account getUsefulById(int accountId) {
		// return dao.findFirst("select " + columns +" from account where id=? and
		// status=? limit 1", accountId, Account.STATUS_OK);
	...
	public Account getById(int accountId) {
		...
		if (account == null) {
			...
			// account = dao.findFirst("select * from account where id=? and status=? limit
			// 1", accountId, Account.STATUS_OK);
	...
	/**
	 * 在 Project、Share、Feedback、NewsFeed 等模块，需要关联查询获取 Account 对象的 nickName、avatar
	 * 时使用此方法 避免使用关联查询，优化性能，在使用中更关键的地方在于缓存的清除
	 * 
	 * @param joinOnField
	 *            join 使用的字段名，account 这端使用 id
	 * @param model
	 *            需要 join 的 model
	 * @param joinAttrs
	 *            需要 join 到 model 中的的属性名称
	 */
	...
	private void updateLikeCount(int accountId, boolean isAdd) {
		String sql = isAdd ? "update account set likeCount=likeCount+1 where id=? limit 1"
