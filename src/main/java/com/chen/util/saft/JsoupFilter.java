...
public class JsoupFilter {
	...
	private static Whitelist createContentWhitelist() {
		return Whitelist.relaxed()
				/**
				 * 必须要删除应用在 a 与 img 上的 protocols，否则就只有使用了这些 protocol 的才不被过滤，比较蛋疼 在 remove
				 * 的时候，后面的 protocols 要完全一个不露的对应上 jsoup 默认已经添加的，否则仍然会被过滤掉 在升级 jsoup 后需要测试这 a 与
				 * img 的过滤是否正常
				 */
				...
				.addAttributes("param", "name", "value").addAttributes("embed", "src", "quality", "width", "height",
						"allowFullScreen", "allowScriptAccess", "flashvars", "name", "type", "pluginspage");
	}

	/**
	 * 过滤 model 中的 title 与 content 字段，其中 title 过滤为纯 text content 使用 contentWhitelist
	 * 过滤
	 */
	...
	/**
	 * 将回车换行符过滤成 <br>
	 * 标记。三次 replace 为兼容 windows、linux、mac os 输入 windows换行为：\r\n linux 换行为：\n mac os
	 * 换行为：\r
	 */
	...
	/**
	 * 使用Whitelist.simpleText() 白名单，获取 simple html 内容 允许的 tag："b", "em", "i",
	 * "strong", "u"
	 */
	...
	/**
	 * 使用Whitelist.simpleText() 白名单，获取的 basic 内容 允许的 tag："a", "b", "blockquote",
	 * "br", "cite", "code", "dd", "dl", "dt", "em", "i", "li", "ol", "p", "pre",
	...
	/**
	 * 使用Whitelist.relaxed() 白名单，获取比较宽松的内容 允许的 tag: "a", "b", "blockquote", "br",
